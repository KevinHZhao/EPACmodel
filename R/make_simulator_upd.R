#' Construct a simulator for a model UPDATED
#'
...
make_simulator_upd <- function(
  ...
  # Load model
  model = get_model_upd(model.name, local = local)
  ...
  before.sim.file.name = get_model_path(
    ...
    file.name = "run-before-simulator_upd.R",
    local = local
  )
  ...
  model_simulator = eval(parse(text = readLines(
    get_model_path(
      model.name = model.name,
      file.name = "simulator-expression_upd.R",
      local = local
    )
  )))
  ...
  after.sim.file.name = get_model_path(
    model.name = model.name,
    file.name = "run-after-simulator_upd.R",
    local = local
  )
