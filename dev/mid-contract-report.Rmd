...
Run this once to ensure you are on the updated fork of `EPACmodel`:
```{r eval=FALSE}
remotes::install_github("KevinHZhao/EPACmodel")
```
...
load("dev/expected_outputs.RData")
...
Instead of `value_type` describing whether the row describes the value of a
state variable or that state variable's total inflow, it now simply refers to
