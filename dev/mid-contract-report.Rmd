...
In order to reproduce this document from source, please follow the following procedure:

**DE:** _The following is not clear to me.  We need to talk about how to rephrase this._

Scripts to recreate expected output objects (already generated), to be run 
on old `EPACmodel` version:
```{r eval=FALSE}
...
Run this once to ensure you are on the updated fork of `EPACmodel` as of writing
this document:
```{r eval=FALSE}
remotes::install_github("KevinHZhao/EPACmodel@e30793b")
```
...
From updating to modern `macpan2`, `macpan2::Model` objects no longer exist,
and `get_model()` now produces `TMBModelSpec` objects.
The creation of this model object has been modified with the intention of being
easier to wield with varying age groups; the default values are now scalars
ignoring age structure, instead of having scalars for each of the age grouped 
state/flow variables.
These default values can be modified when the simulator object is built to be
vectors, as we will later show.
The model expressions have also been simplified, as they use the age grouped
state/flow variable vectors to compute the flows at each time step.
An advantage of these modifications are that now, when building the simulator
object, it would be very simple to modify the code with different age groups
by modifying `age.group.lower` in the `run-before-simulator.R` script/making
it a user specified argument when calling `make_simulator`, and supplying the
relevant `values` appropriate to the age groupings.
...
Before, "transmission" was obtained by running
...
but due to re-ordering in our updates, we have modified this test to run
