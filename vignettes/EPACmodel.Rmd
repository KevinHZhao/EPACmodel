...
When building the simulator, you can specify which flows you would like to be returned when simulating (if none are specified, only state variables will be returned).  To get a list of all flow variables, use the `list_flows()` function:

```{r}
list_flows("old-and-young")
```

To have the simulator also return the number of individuals moving due to the flows `"prog"` (individuals progressing from `E` to `I`) `"d_H"` (individuals moving from `H` to `D`) and `"d_I"` (individuals moving from `I` to `D`) at each time step, we can run:

```{r}
model.name <- "old-and-young"
model_simulator <- make_simulator(
  model.name = model.name,
  output_flows = c("prog", "d_H", "d_I")
)
```
...
- `flow`: flow variables, typically representing number of individuals moving between states at a given time.

The output `variable_name` are the names of state and flow variables, stratified by the relevant age groups.
...
```
To view the number of individuals changing compartments due to the `"prog"` flow:

```{r}
(sim.output
 |> dplyr::filter(variable_name == "prog")
 |> head()
)
```
